...
int	main(int argc, char **argv){
	...
	else if (argc > 1 && argv[1][0] == 'b')
	{
		...
		printf("Returns are\norgi: %d\nmine: %d\n\n", r_origina, r_mine);
								...
								// 4
		r_origina = printf("This is the original printf: |%.12d|\n", -1);
		r_mine = ft_printf("This is the ft vers. printf: |%.12d|\n", -1);
		printf("Returns are\norgi: %d\nmine: %d\n\n", r_origina, r_mine);
								// 5
		r_origina = printf("This is the original printf: |%#x %#x %#x %#x %#x %#x %#x|\n", INT_MAX, INT_MIN, LONG_MAX, LONG_MIN, ULONG_MAX, 0, -42);
		r_mine = ft_printf("This is the ft vers. printf: |%#x %#x %#x %#x %#x %#x %#x|\n", INT_MAX, INT_MIN, LONG_MAX, LONG_MIN, ULONG_MAX, 0, -42);
		printf("Returns are\norgi: %d\nmine: %d\n\n", r_origina, r_mine);
	}
}
